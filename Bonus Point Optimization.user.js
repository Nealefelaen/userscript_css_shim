...
// Agiroth version: use localstorage as GM_get/setValue doesn't work for him
/*var torrentData = JSON.parse(GM_getValue('bpopt', ''));
torrentData.loops = 1;
GM_setValue('bpopt', JSON.stringify(torrentData));
*/
...
// this is the function that is called to calculate the new field
function calculateBPperYearperGB(torrent, coj, cojyears, mightychef, mightychefdays)
{
  if(!coj)
    return torrent.bpyear/(torrent.size/Math.pow(1024, 3));
  ...
  var Q = b / Math.pow(torrent.seeders, c);
  //var t = Math.exp((bpPerHour/((torrent.size/Math.pow(1024, 3))*goldenMultiplier) - a) / Q) - 1.0; // seed time in days
  ...
  var mightychef = goldenMultiplier*6*s*Math.pow(u,(-3/5))*(5*d*Math.pow(u,(3/5)) - 12*d - 12*t*Math.log(t + 1) + 12*(d + t)*Math.log(d + t + 1) - 12*Math.log(t + 1) + 12*Math.log(d + t + 1))/5;
  return mightychef;
}
...
function addLinks()
{
  if(window.location.pathname.indexOf('bonus.php') != -1 || (window.location.pathname.indexOf('bprate.php') != -1 && window.location.href.indexOf('optimization=true') == -1))
  {
    ...
    if(linkbox.length == 0) 
      return; // if the page doesn't have a linkbox, we don't want to add a link there, so we return
...
function loadData(cDiv, messageDiv)
{
  /*var torrentData = JSON.parse(GM_getValue('bpopt', '{}'));
  ...
  xhr.onreadystatechange = xhr_func.bind(undefined, cDiv, messageDiv, xhr, parseData.bind(undefined, cDiv, messageDiv, 1));
...
function xhr_func(cDiv, messageDiv, xhr, func)
{
  if(xhr.readyState == 4)
  {
    if(xhr.status == 200)
...
function parseSize(size)
{
  ...
  return s * Math.pow(1024, p);
}

function parseData(cDiv, messageDiv, page, data)
{
...
function loadSeedingPage(cDiv, messageDiv, torrentData, page)
{
  messageDiv.innerHTML = 'Loading page '+page+' from snatchlist.php';

  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = xhr_func.bind(undefined, cDiv, messageDiv, xhr, parseSeedingData.bind(undefined, cDiv, messageDiv, torrentData, page));
  xhr.open("GET", window.location.origin+"/snatchlist.php?full=1&order_by=seeding&order_way=desc&page="+page);
  xhr.send();
}

function parseSeedingData(cDiv, messageDiv, torrentData, page, data)
{
  var page1 = document.createElement('div');
  page1.innerHTML = data;
...
function showOptimization(cDiv, torrentData)
{
  ...
  for(var i=0; i<torrentData.torrents.length; i++)
  {
    ...
    if(torrentData.useMightychef)
    {
      span.innerHTML = printNumber((calculateMightychefYears(t, torrentData.mightychefdays ? torrentData.mightychefdays : 365)/divisor)/parseFloat(t.size/Math.pow(1024, 3)));
    }
...
function dumpData(torrents)
{
  document.body.innerHTML=JSON.stringify(torrents);
}

function addCSVtoLink(a, torrentData)
{
...
function actualAddCSV(a, torrentData, index, text, fields)
{
  if(!a.parentNode)
    return;
...
function getSortFunc(sortBy, existingSortBy, coj, mightychef, mightychefdays)
{
...
{
  var aVal = calculateMightychefYears(a, days)/parseFloat(a.size/Math.pow(1024, 3));
  var bVal = calculateMightychefYears(b, days)/parseFloat(b.size/Math.pow(1024, 3));
...
function hideTorrent(i, cDiv, torrentData)
{
  var t = torrentData.torrents[i];
  t.hidden = !t.hidden;
  //GM_setValue('bpopt', JSON.stringify(torrentData));
...
function showTorrent(i, cDiv, torrentData)
{
...
function useMightychef(cDiv, torrentData, a)
{
  if(torrentData.useMightychef)
  {
    a.innerHTML = "Not using mightychef's algorithm";
...
function useCoj(cDiv, torrentData, a)
{
  if(torrentData.useCoj)
  {
    a.innerHTML = "Not using coj's algorithm";
...
function applyCojYears(cDiv, torrentData, input)
{
...
function applyMightychefYears(cDiv, torrentData, input)
{
...
function applyMinimumSeedTime(cDiv, torrentData, input)
{
...
function calculateMightychefYears(torrent, days)
{
  return calculateBPperYearperGB(torrent, true, 1, true, days);
}

function applyDivisor(cDiv, torrentData, input)
{
...
function applyMightychefTarget(cDiv, torrentData, input)
{
...
function invertHidden(cDiv, torrentData)
{
  for(var i=0; i<torrentData.torrents.length; i++)
  {
...
function showAllTorrents(cDiv, torrentData)
{
  for(var i=0; i<torrentData.torrents.length; i++)
  {
    torrentData.torrents[i].hidden = false;
...
function showHidden(cDiv, torrentData)
{
...
function showOptions(cDiv, torrentData)
{
...
function hideGP(cDiv, torrentData)
{
  for(var i=0; i<torrentData.torrents.length; i++)
  {
    var t = torrentData.torrents[i];
...
function hideNeedToSeed(cDiv, torrentData)
{
  for(var i=0; i<torrentData.torrents.length; i++)
  {
    var t = torrentData.torrents[i];
...
function hideRatioLessThanOne(cDiv, torrentData)
{
  for(var i=0; i<torrentData.torrents.length; i++)
  {
    var t = torrentData.torrents[i];
...
function hideFewSeeders(cDiv, torrentData)
{
  for(var i=0; i<torrentData.torrents.length; i++)
  {
    var t = torrentData.torrents[i];
...
function printNumber(number, unfixed)
{
  if(!unfixed)
...
function loadPage(cDiv, messageDiv, page)
{
  messageDiv.innerHTML = 'Loading page '+(page)+' from bprate.php';

  var xhr = new XMLHttpRequest();
  xhr.onreadystatechange = xhr_func.bind(undefined, cDiv, messageDiv, xhr, parseData.bind(undefined, cDiv, messageDiv, page));
